...
const Slider = () => {
  ...
  useEffect(() => {
    try {
      ...
      const totalSlides = latestProjects.length;
      const stripCount = 25;
      ...
      for (let i = 1; i < totalSlides; i++) {
        ...
        for (let j = 0; j < stripCount; j++) {
          ...
          img.src = latestProjects[i].mainImage;
          img.alt = latestProjects[i].title;
      ...
      function animateTitleChange(index, direction) {
        ...
        if (index < 0 || index >= latestProjects.length) return;
        if (isAnimating) {
      ...
      function getTitleINdexForProgress(imageProgress) {
        ...
        if (imageSpecificProgress >= titleChangeThreeshold)
          return Math.min(imageIndex + 1, latestProjects.length - 1);
        else {
          return imageIndex;
        }
      }
